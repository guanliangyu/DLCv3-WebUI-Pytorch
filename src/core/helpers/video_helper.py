...
def build_ffmpeg_reencode_command(input_path: str, output_path: str) -> List[str]:
    """Build ffmpeg command arguments for MP4 re-encoding."""
    return [
        "ffmpeg",
        "-y",
        "-i",
...
def run_ffmpeg_reencode(input_path: str, output_path: str) -> None:
    """Run ffmpeg re-encoding command without invoking shell interpolation."""
    subprocess.run(
        build_ffmpeg_reencode_command(input_path, output_path),
        check=True,
...
def get_video_info(video_path):
    ...
    try:
        ...
        if fps == 0:
            st.warning(
                f"视频帧率获取失败，使用默认值30fps / Failed to get video FPS, using default value 30fps"
            )
            fps = 30
...
def crop_video_files(
    ...
    for video_path in selected_files:
        try:
            ...
            # 使用ffmpeg重新编码以确保兼容性
            try:
                run_ffmpeg_reencode(temp_output, output_path)
            finally:
